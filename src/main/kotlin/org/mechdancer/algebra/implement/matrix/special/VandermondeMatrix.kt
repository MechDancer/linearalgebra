...
class VandermondeMatrix private constructor(elements: List<Double>)
	: Matrix by listMatrixOf(elements.size, elements.size, { r, c -> elements[c].pow(r) }) {

	override val det: Double =
		if (elements.size >= 2) {
			elements.indices.reversed().fold(1.0) { acc, i ->
				acc * (i - 1 downTo 0).fold(1.0) { sum, j ->
					sum * (elements[i] - elements.getOrElse(j) { .0 })
				}
			}
	...
	companion object {
		operator fun get(vararg element: Double) = VandermondeMatrix(element.toList())
	}
}