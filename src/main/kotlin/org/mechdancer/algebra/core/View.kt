...
fun tie(vararg items: Any?): String {
    ...
    val widths = list.associateWith { it.firstOrNull()?.length }
    val row = list.asSequence().map(List<*>::size).max() ?: 0
 ...
 * 方程组显示为增广矩阵
 */
fun AugmentedMatrix.matrixView(header: String = "${args.column} members equation set") =
    buildString {
        val lines =
            tie(args.matrixView(""),
