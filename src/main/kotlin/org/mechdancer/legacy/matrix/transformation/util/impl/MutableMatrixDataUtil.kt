package org.mechdancer.legacy.matrix.transformation.util.impl
...
class MutableMatrixDataUtil(data: MatrixData) : MatrixDataUtil {
	...
	fun addRow(data: MatrixElement) =
		apply { mutableData.add(data.toMutableList()) }

	fun addColumn(data: MatrixElement) =
		apply {
	...
	override fun removeRow(row: Int): MatrixDataUtil =
		apply { mutableData.removeAt(row) }

	override fun removeColumn(column: Int): MatrixDataUtil =
		apply {
	...
	): MatrixDataUtil = apply {
		if (elements.size != mutableData.first().size)
			throw IllegalArgumentException("元素项数不同")
		checkRowRange(row)
		mutableData[row] = elements.toMutableList()
	}

	override fun replaceColumn(
		column: Int,
		elements: MatrixElement
	): MatrixDataUtil = apply {
		if (elements.size != mutableData.size)
			throw IllegalArgumentException("元素项数不同")
	...
	private fun checkRowRange(row: Int) =
		if (row !in mutableData.indices)
			throw rowIndexError
		else Unit


	private fun checkColumnRange(column: Int) =
		if (column !in 0 until mutableData.first().size)
			throw columnIndexError
		else Unit
}

fun operateMatrixDataMutable(
	data: MatrixData,
	block: MutableMatrixDataUtil.() -> Unit
